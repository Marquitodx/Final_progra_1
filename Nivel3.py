...
class Nivel_3:
    ...
    def __init__(self, pantalla: pygame.Surface) -> None:
        ...
        self.FONDO = pygame.image.load(FONDO_NIVEL_3).convert_alpha()
        ...
        self.lista_enemigos = lista_de_enemigos_nivel_3
        self.tiempo_creacion_enemigo = pygame.USEREVENT + 1
        pygame.time.set_timer(self.tiempo_creacion_enemigo, 10000)

        self.plataformas = plataformas_nivel_3
        self.lista_monedas = lista_de_monedas_nivel_3
    ...
    def update(self, lista_eventos):
            piso_nuevo = random.choice(plataformas_para_enemigos_nivel_3)
            ...
            for evento in lista_eventos:
                ...
                self.leer_inputs(lista_eventos,
                                self.tiempo_creacion_enemigo,
                                self.lista_enemigos,
                                Enemigo(piso_nuevo),
                                Vida(TUERCA_VIDA, self.ANCHO),
                                self.lista_vidas)
            ...
            tiempo_agotado = self.relojes_cronometro(self.tiempo_inicial,
                                                    self.duracion_milisegundos,
            ...
            self.jugador.actualizar(self._slave, self.plataformas,
                                    self.lista_enemigos, self.jugador, 
                                    self.FUENTE_GRANDE, self.BLANCO,
                                    self.ROJO, self.superficie_opaca, 
                                    tiempo_agotado)
            ...
            if get_mode():
                self.pintar_lineas(self._slave, self.jugador, 
    ...
    def pintar_lineas(self, pantalla, personaje, 
                    plataformas, enemigos, lista_monedas):
